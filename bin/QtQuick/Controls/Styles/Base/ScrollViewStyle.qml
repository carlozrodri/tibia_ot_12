    ...
    */
    ...
    property Component __scrollbar: Item {
        ...
        Loader {
            ...
            anchors.topMargin: __styleData.horizontal || range === 0 ? 0 : Math.floor(-handleOverlap + (2 * begin * (bg.height + (2 * handleOverlap) - extent) + range) / (2 * range))
            anchors.leftMargin: __styleData.horizontal && range !== 0 ? Math.floor(-handleOverlap + (2 * begin * (bg.width + (2 * handleOverlap) - extent) + range) / (2 * range)) : 0
